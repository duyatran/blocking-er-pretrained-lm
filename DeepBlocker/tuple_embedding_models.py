...
class SIFEmbedding(ABCTupleEmbedding):
    ...
    def __init__(self, word_embedding_model, sif_weighting_param=1e-3, remove_pc=True, min_freq=0):
        super().__init__()
        print("Loading FastText model")
...
class AutoEncoderTupleEmbedding(ABCTupleEmbedding):
    def __init__(self, word_embedding_model, hidden_dimensions=(2*AE_EMB_DIMENSION_SIZE, AE_EMB_DIMENSION_SIZE)):
        super().__init__()
        self.input_dimension = EMB_DIMENSION_SIZE
        self.hidden_dimensions = hidden_dimensions
        self.sif_embedding_model = SIFEmbedding(word_embedding_model)
...
class CTTTupleEmbedding(ABCTupleEmbedding):
    ...
    def get_tuple_embedding(self, list_of_tuples):
        embedding_matrix = torch.tensor(self.sif_embedding_model.get_tuple_embedding(list_of_tuples)).float()
        # TODO: is this a bug? return self.ctt_model.get_tuple_embedding(embedding_matrix)?
...
class HybridTupleEmbedding(ABCTupleEmbedding):
    def __init__(self, hidden_dimensions=(2*AE_EMB_DIMENSION_SIZE, AE_EMB_DIMENSION_SIZE),
        ...
        # BUG FIX: dimension mismatch
        # Hybrid uses autoencoder instead of SIF aggregator
        # self.autoencoder_embedding_model = AutoEncoderTupleEmbedding(hidden_dimensions=(EMB_DIMENSION_SIZE, EMB_DIMENSION_SIZE))
    ...
    def preprocess(self, list_of_tuples):
        ...
        print("Training Hybrid model - Autoencoder for aggregation")
    ...
    def get_tuple_embedding(self, list_of_tuples):
        ...
        # TODO: is this a bug? CTT trained not used?
        return embedding_matrix
        # return self.ctt_model.get_tuple_embedding(embedding_matrix)
